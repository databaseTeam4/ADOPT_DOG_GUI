...
class DB_Conn_Query {
	...
	public ArrayList<dog_join_protect> read_abandoned_dog() {
		ArrayList<dog_join_protect> arr1 = new ArrayList<dog_join_protect>();
		try {
			Oracle_Connect();
			String sql1 = "select 유기견번호,유기견이름,유기견품종,성별,유기견.보호기관번호,보호기관명,주소 from 유기견 join 보호소 on 유기견.보호기관번호 = 보호소.보호기관번호";
			st1 = con.createStatement();
			rs1 = st1.executeQuery(sql1);

			while (rs1.next()) {
				arr1.add(new dog_join_protect(rs1.getInt(1), rs1.getString(2), rs1.getString(3), rs1.getString(4),
	...
	public ArrayList<dog_join_protect> p_local_abandoned_dog(String region) {
		ArrayList<dog_join_protect> arr11 = new ArrayList<dog_join_protect>();
		...
		try {
			...
			// 해당 지역 유기견 목록 출력
			while (rs.next()) {
				arr11.add(new dog_join_protect(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
	...
	public ArrayList<member> member_sqlrun() {
		...
		try {
			...
			while (rs7.next()) {
				arr7.add(new member(rs7.getString(1), rs7.getString(2), rs7.getInt(3), rs7.getString(4),
						rs7.getString(5)));
			}
	...
	// 새 유기견 정보 등록
	public void insert_abandoned_dog(String name, String kind, String gender, String age, String weight,
			String rescure_date, String discover_place, String protection_agegncy) {
		try {
			int dog_num = 0;
			int prot_num = 0;
			Oracle_Connect();
			String sql1 = "select 유기견번호,보호등록번호 from 유기견 where 유기견번호 = (select MAX(유기견번호) from 유기견)"; // 유기견 번호 증가
			pst = con.prepareStatement(sql1);
			rs8 = pst.executeQuery();
			while (rs8.next()) {
				dog_num = rs8.getInt(1) + 1;
				prot_num = Integer.parseInt(rs8.getString(2).substring(4))+1; // 보호등록번호 자동화 추가
			}
	...
	public ArrayList<adopt> read_adopt(String year, String month) {
		...
		try {
			Oracle_Connect();
			// 해당 한 달 내 입양신청 목록 sql문->Select * from 입양신청 where 입양신청일자 between
			// TO_DATE('2022/11/01','YYYY/MM/DD') And TO_DATE('2022/11/30','YYYY/MM/DD');
			String sql1 = "Select * from 입양신청 where 입양신청일자 between TO_DATE('" + date + "','YYYY/MM/DD') And TO_DATE('"
					+ sql2 + "','YYYY-MM-DD')";
			st9 = con.createStatement();
			rs10 = st9.executeQuery(sql1);
			while (rs10.next()) {
				arr.add(new adopt(rs10.getString(1), rs10.getDate(2), rs10.getString(3), rs10.getString(4),
						rs10.getString(5), rs10.getString(6), rs10.getString(7)));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
