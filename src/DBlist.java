...
public class DBlist {
	...
	private String colName1[] = { "회원ID", "이름", "나이" }; // 컬럼명을 배열에 선언
    public DefaultTableModel model = new DefaultTableModel(colName1, 0) {
    	public boolean isCellEditable(int rowIndex, int mColIndex) {
    		return false;
    	}
    }; //
    private String colName2[] = { "반려견 이름", "반려견 품종", "실종된 날짜","회원 아이디"}; // 컬럼명을 배열에 선언
    public DefaultTableModel model1 = new DefaultTableModel(colName2, 0) {
    	public boolean isCellEditable(int rowIndex, int mColIndex) {
    		return false;
    	}
    }; //
    private String colName3[] = { "1", "2", "3","4","5" }; // 컬럼명을 배열에 선언
